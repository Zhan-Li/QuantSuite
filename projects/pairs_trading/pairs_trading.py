...
random_grid ={
        ...
        'entry': tune.sample_from(lambda spec: tune.quniform(0.01, 0.1, 0.01) if spec.config.signal == 'ecdf' else tune.quniform(1.5, 3, 0.1)),
        'exit': tune.sample_from(lambda spec:  tune.quniform(0.4, 0.6, 0.01) if spec.config.signal == 'ecdf' else tune.quniform(-1, 1, 0.1))
    }
    ...
    while True:
        ...
        if cutoff_date <= max_date:
            ...
            pt = PairsTrading(position_value=10000,
                              hr_test=hr_test, hr_trade =  hr_trade, hr_window=OLS_window, hr_min_share_decimal=4, hr_constant=constant,
                              perf_signal=signal, perf_signal_window=signal_window, perf_signal_entry=entry, perf_signal_exit=exit,
...
for count, value in enumerate([train_val, test]):
    rs = objective(**my_param, data=value, n_job=-1, save_perf=True)['rs']

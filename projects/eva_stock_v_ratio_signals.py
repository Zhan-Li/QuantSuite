...
QQQs = DataManipulator(QQQs, 'Date', 'ticker')\
    ...
    .withColumn('position_raito', f.when(f.col('LS_ratio')>cutoff, 1).when(f.col('LS_ratio')<-cutoff, -1).otherwise(0))\
...
# retest
r_train = objective(**best_params, crsp_data=train_data, sig_rank_selected='high', report=True, filename='report_train.html', trading_cost=True)
r_test = objective(**best_params, crsp_data=test_data, sig_rank_selected='high', report=True, filename='report_test.html', trading_cost=True)
# detailed analysis
days = 1
start = -75
end = -1
